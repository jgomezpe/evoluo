...
public abstract class HaeaOperators<T> implements UsesRawGenerator{
    ...
    public void reward(double[] r, int operIndex) {
        if( operIndex >= 0 ){
            r[operIndex] += (1.0 - r[operIndex]) * getRaw().next();
            Util.normalize(r);
        }
    }
    ...
    public void punish(double[] r, int operIndex) {
        if( operIndex >= 0 ){
            r[operIndex] -= r[operIndex] * getRaw().next();
            Util.normalize(r);
        }    
    }
    ...
    public void resize( int n ){
        ...
        if( m < n ){
            for( int i=m; i<n; i++){
                double[] r = getRaw().raw( numberOfOperatorsPerIndividual() );
                Util.normalize(r);
                rates.add(r);            
                sel_oper.add(-1);
     ...
     */
    public double[] rates( int indIndex ){ return rates.get(indIndex); }
